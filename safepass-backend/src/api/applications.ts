...
router.post('/worker/applications', authenticateToken, requireRole(['Worker']), async (req: Request, res: Response) => {
  try {
    const currentUser = req.user!;
...
router.put('/agency/applications/:applicationId', authenticateToken, requireRole(['AgencyAdmin']), async (req: Request, res: Response) => {
  try {
    const currentUser = req.user!;
    ...
    const { data: applicationJob, error: fetchError } = await supabase
      .from('job_applications')
      .select(`
        id,
        job_id,
    ...
    if ((applicationJob.jobs as any).agency_id !== currentUser.id) {
    ...
    const { data: fullApplication, error: detailsError } = await supabase
      .from('applications_with_details')
      .select('*')
      .eq('id', application.id)
      .single();
