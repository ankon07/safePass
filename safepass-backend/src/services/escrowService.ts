...
export class EscrowService {
   ...
   */
  async getEscrowDeposit(employmentContractAddress: string): Promise<EscrowDeposit | null> {
    const { data, error } = await supabase
      .from('escrow_deposits')
      .select('*')
   ...
   */
  async getEmployerEscrows(employerAddress: string): Promise<EscrowDeposit[]> {
    const { data, error } = await supabase
      .from('escrow_deposits')
      .select('*')
   ...
   */
  async getWorkerEscrows(workerAddress: string): Promise<EscrowDeposit[]> {
    const { data, error } = await supabase
      .from('escrow_deposits')
      .select('*')
  ...
  async updateEscrowStatus(
    employmentContractAddress: string,
   ...
   */
  async getActiveEscrows(): Promise<EscrowDeposit[]> {
    const { data, error } = await supabase
      .from('escrow_deposits')
      .select('*')
      .eq('status', 'Active')
   ...
   */
  async getDisputedEscrows(): Promise<EscrowDeposit[]> {
    const { data, error } = await supabase
      .from('escrow_deposits')
      .select('*')
      .eq('status', 'Disputed')
   ...
   */
  async getEmployerTotalEscrowed(employerAddress: string): Promise<number> {
    const { data, error } = await supabase
      .from('escrow_deposits')
   ...
   */
  async getEscrowsNeedingAttention(daysOld: number = 90): Promise<EscrowDeposit[]> {
   ...
   */
  calculateEscrowRequirement(salary: number, payFrequencyDays: number): number {
   ...
   */
  async checkEscrowCompliance(employmentContractAddresses: string[]): Promise<{
    compliant: string[];
    nonCompliant: string[];
    details: Record<string, { hasEscrow: boolean; amount: number; status: string }>;
  }> {
    const compliant: string[] = [];
    const nonCompliant: string[] = [];
    const details: Record<string, { hasEscrow: boolean; amount: number; status: string }> = {};
