...
class TrustScoreService extends events_1.EventEmitter {
    constructor() {
        super();
        this.config = null;
    ...
    async loadConfiguration() {
        try {
            ...
            this.config = data.reduce((acc, item) => {
                acc[item.parameter_name] = parseFloat(item.parameter_value);
                return acc;
            }, {});
     ...
     */
    async calculateAllAgencyScores() {
     ...
     */
    async calculateAgencyScore(agencyAddress) {
        if (!this.config) {
     ...
     */
    async recordTrustScoreEvent(event) {
        try {
     ...
     */
    async getAgencyTrustScore(agencyAddress) {
        try {
            const { data, error } = await supabase_1.supabase
    ...
    async getAllRegisteredAgencies() {
        try {
            // Try to get from blockchain first
            if (blockchainService_1.blockchainService.getContract && blockchainService_1.blockchainService.getContract('AgencyRegistry')) {
                const agencies = await blockchainService_1.blockchainService.callContractMethod('AgencyRegistry', 'getAllAgencies');
                return agencies;
            }
    ...
    async getAgencyTrustData(agencyAddress) {
        try {
            const { data, error } = await supabase_1.supabase
                .from('agency_trust_scores')
                .select('*')
    ...
    async getNewTrustScoreEvents(agencyAddress) {
        try {
            const { data, error } = await supabase_1.supabase
                .from('trust_score_events')
                .select('*')
    ...
    async storeTrustScore(agencyAddress, agencyDid, scoreData) {
        try {
            const { error } = await supabase_1.supabase
                .from('agency_trust_scores')
                .insert({
    ...
    async updateBlockchainTrustScore(agencyAddress, newScore) {
        try {
            if (!blockchainService_1.blockchainService.getContract) {
                return;
            }
            const agencyRegistry = blockchainService_1.blockchainService.getContract('AgencyRegistry');
            if (!agencyRegistry) {
                console.log('AgencyRegistry contract not available, skipping blockchain update');
                return;
            }
            // Check if agency is registered on blockchain
            const isRegistered = await blockchainService_1.blockchainService.callContractMethod('AgencyRegistry', 'isAgencyRegistered', [agencyAddress]);
            if (isRegistered) {
                await blockchainService_1.blockchainService.executeContractMethod('AgencyRegistry', 'updateTrustScore', [agencyAddress, newScore]);
