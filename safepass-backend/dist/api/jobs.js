...
router.get('/agency/jobs', auth_1.authenticateToken, (0, auth_1.requireRole)(['AgencyAdmin']), async (req, res) => {
    try {
...
router.post('/agency/jobs', auth_1.authenticateToken, (0, auth_1.requireRole)(['AgencyAdmin']), async (req, res) => {
    try {
        const currentUser = req.user;
        const { title, description, requirements, salary, location, jobType, category, expiresAt, benefits, workingHours, accommodationProvided, transportationProvided, visaSponsorship } = req.body;
        // Validation
...
router.put('/agency/jobs/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['AgencyAdmin']), async (req, res) => {
    try {
        const currentUser = req.user;
        const { id } = req.params;
        const updateData = req.body;
        // First check if the job belongs to the current agency
        const { data: existingJob, error: fetchError } = await supabase_1.supabase
            .from('jobs')
            .select('agency_id')
            .eq('id', id)
            .single();
        ...
        if (updateData.requirements)
            dbUpdateData.requirements = updateData.requirements;
        if (updateData.salary) {
            if (updateData.salary.min)
                dbUpdateData.salary_min = updateData.salary.min;
            if (updateData.salary.max)
                dbUpdateData.salary_max = updateData.salary.max;
            if (updateData.salary.currency)
                dbUpdateData.salary_currency = updateData.salary.currency;
        }
        if (updateData.location) {
            if (updateData.location.country)
                dbUpdateData.location_country = updateData.location.country;
            if (updateData.location.city)
                dbUpdateData.location_city = updateData.location.city;
        }
        if (updateData.jobType)
            dbUpdateData.job_type = updateData.jobType;
        if (updateData.category)
            dbUpdateData.category = updateData.category;
        if (updateData.expiresAt)
            dbUpdateData.expires_at = updateData.expiresAt;
        if (updateData.status)
            dbUpdateData.status = updateData.status;
        if (updateData.benefits)
            dbUpdateData.benefits = updateData.benefits;
        if (updateData.workingHours)
            dbUpdateData.working_hours = updateData.workingHours;
        if (updateData.accommodationProvided !== undefined)
            dbUpdateData.accommodation_provided = updateData.accommodationProvided;
        if (updateData.transportationProvided !== undefined)
            dbUpdateData.transportation_provided = updateData.transportationProvided;
        if (updateData.visaSponsorship !== undefined)
            dbUpdateData.visa_sponsorship = updateData.visaSponsorship;
        const { data: job, error } = await supabase_1.supabase
            .from('jobs')
            .update(dbUpdateData)
            .eq('id', id)
...
router.delete('/agency/jobs/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['AgencyAdmin']), async (req, res) => {
    try {
        const currentUser = req.user;
        const { id } = req.params;
        // First check if the job belongs to the current agency
