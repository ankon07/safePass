...
router.get('/agencies/:address', async (req, res) => {
    try {
        ...
        const trustScore = await trustScoreService_1.trustScoreService.getAgencyTrustScore(address);
        if (trustScore === null) {
...
router.get('/statistics', async (req, res) => {
    try {
        const statistics = await trustScoreService_1.trustScoreService.getTrustScoreStatistics();
...
router.get('/top-agencies', async (req, res) => {
    try {
        ...
        const topAgencies = await trustScoreService_1.trustScoreService.getTopAgencies(limit);
...
router.post('/calculate', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (agency_address) {
            ...
            const newScore = await trustScoreService_1.trustScoreService.calculateAgencyScore(agency_address);
            res.json({
            ...
            await trustScoreService_1.trustScoreService.calculateAllAgencyScores();
            res.json({
 ...
 */
router.post('/events', auth_1.authenticateToken, async (req, res) => {
    try {
        // Check if user is a regulator
        if (req.user?.role !== 'Regulator') {
            return res.status(403).json({
        ...
        const { agency_address, event_type, contract_address, employment_contract_id, impact_score, event_data } = req.body;
        ...
        await trustScoreService_1.trustScoreService.recordTrustScoreEvent({
            agency_address,
            event_type,
            contract_address,
