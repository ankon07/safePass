 ...
 */
router.get('/agency/documents/workers', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const workerDocuments = documents?.filter(doc => doc.worker && doc.worker.role === 'Worker') || [];
        // Transform the data
 ...
 */
router.get('/agency/documents/pending', auth_1.authenticateToken, async (req, res) => {
    try {
        // Verify user is an agency admin
        if (!req.user || req.user.role !== 'AgencyAdmin') {
            return res.status(403).json({
 ...
 */
router.post('/agency/documents/approve', auth_1.authenticateToken, async (req, res) => {
    try {
        // Verify user is an agency admin
        if (!req.user || req.user.role !== 'AgencyAdmin') {
            return res.status(403).json({
 ...
 */
router.post('/agency/documents/reject', auth_1.authenticateToken, async (req, res) => {
    try {
        // Verify user is an agency admin
        if (!req.user || req.user.role !== 'AgencyAdmin') {
            return res.status(403).json({
 ...
 */
router.get('/agency/documents/statistics', auth_1.authenticateToken, async (req, res) => {
    try {
        // Verify user is an agency admin
        if (!req.user || req.user.role !== 'AgencyAdmin') {
            return res.status(403).json({
        ...
        const { data: allDocuments, error: dbError } = await supabase_1.supabase
            .from('document_uploads')
            .select(`
                status,
                created_at,
        ...
        const workerDocuments = allDocuments?.filter(doc => doc.users && doc.users.role === 'Worker') || [];
        ...
        const recentDocuments = workerDocuments.filter(doc => new Date(doc.created_at) >= thirtyDaysAgo).length;
        res.json({
