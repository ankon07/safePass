...
function ZKPManagement() {
  ...
  const fetchZKPData = async () => {
    try {
      ...
      const [statusResponse, proofsResponse, licensesResponse] = await Promise.all([
        apiClient.checkZKPSystemStatus().catch(() => ({ system_ready: false, message: "System not ready" })),
        apiClient.getAgencyZKPProofs().catch(() => ({ proofs: [], count: 0 })),
        apiClient.getValidLicenses().catch(() => ({ valid_licenses: [], count: 0 })),
      ]);

      setSystemStatus({
        initialized: statusResponse.system_ready || false,
        circuitReady: statusResponse.system_ready || false,
  ...
  return (
    <div className="p-4 md:p-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600 mt-2">
              View regulator-generated proofs and verify privacy-preserving license proofs
            </p>
          </div>
      ...
      {verificationResult && (
        <Alert variant={verificationResult.is_valid ? "default" : "destructive"} className="mb-6">
          ...
          <AlertDescription>
            Proof verification: {verificationResult.is_valid ? "Valid" : "Invalid"} - {verificationResult.message}
          </AlertDescription>
        </Alert>
      )}

      {/* Regulator-Controlled System Notice */}
      <Card className="mb-8 border-blue-200 bg-blue-50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-blue-900">
            <Shield className="h-5 w-5" />
            Enhanced Security: Regulator-Controlled Proofs
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-blue-800 space-y-3">
            <p>
              <strong>Important Update:</strong> For enhanced security and trust, ZKP proofs are now generated by regulators on behalf of agencies.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h4 className="font-medium mb-2">How it works:</h4>
                <ul className="text-sm space-y-1">
                  <li>• Regulators verify your license authenticity</li>
                  <li>• Regulators generate ZKP proofs for your agency</li>
                  <li>• You receive regulator-signed proofs to share</li>
                  <li>• Workers trust proofs from verified regulators</li>
                </ul>
              </div>
              <div>
                <h4 className="font-medium mb-2">Benefits:</h4>
                <ul className="text-sm space-y-1">
                  <li>• Prevents fraudulent proof generation</li>
                  <li>• Increases worker confidence</li>
                  <li>• Maintains license privacy</li>
                  <li>• Regulatory oversight and compliance</li>
                </ul>
              </div>
            </div>
            <div className="bg-blue-100 p-3 rounded-lg mt-4">
              <p className="text-sm">
                <strong>Contact your regulator</strong> to request ZKP proof generation for your agency license. 
                Once generated, you can share the proof ID with workers for verification.
              </p>
            </div>
          </div>
        </CardContent>
