...
describe("Anchor", function () {
  ...
  describe("Anchoring Batches", function () {
    ...
    it("Should prevent non-owner from anchoring batches", async function () {
      await expect(
        anchor.connect(nonOwner).anchorNewBatch(merkleRoot)
      ).to.be.revertedWithCustomError(anchor, "OwnableUnauthorizedAccount")
        .withArgs(nonOwner.address);

      await expect(
        anchor.connect(randomUser).anchorNewBatch(merkleRoot)
      ).to.be.revertedWithCustomError(anchor, "OwnableUnauthorizedAccount")
    ...
    it("Should prevent anchoring zero merkle root", async function () {
      ...
      await expect(
        anchor.connect(owner).anchorNewBatch(zeroRoot)
  ...
  describe("Merkle Root Queries", function () {
    ...
    it("Should revert getLatestBatch when no batches exist", async function () {
      ...
      await expect(
        newAnchor.getLatestBatch()
      ).to.be.revertedWith("No batches anchored yet");
    });
  ...
  describe("Access Control", function () {
    ...
    it("Should allow owner to transfer ownership", async function () {
      ...
      await expect(
        anchor.connect(owner).anchorNewBatch(merkleRoot)
      ).to.be.revertedWithCustomError(anchor, "OwnableUnauthorizedAccount")
