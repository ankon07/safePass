...
describe("EmploymentContract", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should revert with zero addresses", async function () {
      ...
      await expect(
        EmploymentContractFactory.deploy(
          ethers.ZeroAddress,
          employer.address,
          salary,
          termsHash,
      ...
      ).to.be.revertedWith("Worker address cannot be zero");
      ...
      ).to.be.revertedWith("Employer address cannot be zero");
      ...
      ).to.be.revertedWith("Regulator address cannot be zero");
    });
    ...
    it("Should revert when worker and employer are the same", async function () {
      ...
      ).to.be.revertedWith("Worker and employer must be different");
    });
    ...
    it("Should revert with empty terms hash", async function () {
      ...
      ).to.be.revertedWith("Terms hash cannot be empty");
    });
  });
  ...
  describe("Contract Signing", function () {
    ...
    it("Should prevent non-parties from signing", async function () {
      await expect(
        employmentContract.connect(randomUser).signContract()
    ...
    it("Should prevent signing when not in Proposed status", async function () {
      ...
      await expect(
        employmentContract.connect(worker).signContract()
      ).to.be.revertedWith("Contract must be in proposed state");
    });
  });
  ...
  describe("Payment Recording", function () {
    ...
    it("Should prevent non-employer from recording payments", async function () {
      ...
      await expect(
        employmentContract.connect(worker).recordPayment(paymentAmount)
      ).to.be.revertedWith("Only employer can call this function");

      await expect(
        employmentContract.connect(randomUser).recordPayment(paymentAmount)
      ).to.be.revertedWith("Only employer can call this function");
    });

    it("Should prevent recording zero payments", async function () {
      await expect(
        employmentContract.connect(employer).recordPayment(0)
    ...
    it("Should prevent recording payments when not active", async function () {
      ...
      const EmploymentContractFactory = await ethers.getContractFactory("EmploymentContract");
      const newContract = await EmploymentContractFactory.deploy(
        worker.address,
        employer.address,
        salary,
        termsHash,
      ...
      await expect(
        newContract.connect(employer).recordPayment(paymentAmount)
  ...
  describe("Dispute Management", function () {
    ...
    it("Should prevent employer from raising disputes", async function () {
      await expect(
        employmentContract.connect(employer).raiseDispute()
      ).to.be.revertedWith("Only worker can call this function");
    });

    it("Should prevent non-parties from raising disputes", async function () {
      await expect(
        employmentContract.connect(randomUser).raiseDispute()
      ).to.be.revertedWith("Only worker can call this function");
    });
    ...
    it("Should prevent raising disputes when not active", async function () {
      ...
      await expect(
        newContract.connect(worker).raiseDispute()
      ).to.be.revertedWith("Contract must be active");
    });
    ...
    it("Should prevent non-regulator from resolving disputes", async function () {
      ...
      await expect(
        employmentContract.connect(worker).resolveDispute(1)
      ).to.be.revertedWith("Only regulator can call this function");

      await expect(
        employmentContract.connect(employer).resolveDispute(1)
      ).to.be.revertedWith("Only regulator can call this function");

      await expect(
        employmentContract.connect(randomUser).resolveDispute(1)
      ).to.be.revertedWith("Only regulator can call this function");
    });

    it("Should prevent resolving when no dispute exists", async function () {
      await expect(
        employmentContract.connect(regulator).resolveDispute(1)
      ).to.be.revertedWith("No active dispute to resolve");
    });
    ...
    it("Should prevent invalid resolution statuses", async function () {
      ...
      await expect(
        employmentContract.connect(regulator).resolveDispute(0) // Proposed
      ).to.be.revertedWith("Invalid resolution status");

      await expect(
        employmentContract.connect(regulator).resolveDispute(2) // Disputed
      ).to.be.revertedWith("Invalid resolution status");
    });
  });
  ...
  describe("Contract Completion", function () {
    ...
    it("Should prevent non-parties from completing", async function () {
      await expect(
        employmentContract.connect(randomUser).completeContract()
      ).to.be.revertedWith("Only contract parties can call this function");
    });
    ...
    it("Should prevent completion from Proposed state", async function () {
      ...
      await expect(
        newContract.connect(worker).completeContract()
      ).to.be.revertedWith("Contract must be active or disputed to complete");
    });
  });
  ...
  describe("Payment Queries", function () {
    ...
    it("Should revert for invalid payment index", async function () {
      await expect(
        employmentContract.getPayment(5)
